...
async def add_product_start(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text("Введите ID категории (существующей):")
...
async def product_cat(message: Message, state: FSMContext):
    ...
    await message.answer("Введите название товара (одной строкой):")
...
async def product_qty(message: Message, state: FSMContext):
    try:
...
async def product_price_input(message: Message, state: FSMContext):
    ...
    await message.answer("Отправьте содержимое товара ТЕКСТОМ (можно много строк). Это то что получит покупатель:")
...
async def product_content(message: Message, state: FSMContext):
    ...
    await message.answer(f"✅ Товар '{data['name']}' добавлен! Количество: {data['quantity']} шт., Цена: {data['price']}$")
    await state.clear()
