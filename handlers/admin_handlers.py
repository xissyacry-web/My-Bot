...
from database.models import User, Product, Category, Promocode, UnbanRequest, Invoice, Purchase
...
async def del_prod_exec(callback: CallbackQuery, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        else: await callback.answer("Товар не найден")
    await state.clear()
...
async def del_cat_exec(callback: CallbackQuery, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not cat: await callback.answer("Категория не найдена"); await state.clear(); return
...
async def user_bal_amount(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if user:
            ...
            try: await message.bot.send_message(uid, f"💰 Ваш баланс {action} на {abs(amount):.2f}$ администратором.\nТекущий баланс: {user.balance:.2f}$")
            except: pass
    ...
    async with AsyncSessionLocal() as session:
        ...
        if user:
            if reason.strip() == '-':
                ...
                try: await message.bot.send_message(uid, f"🚫 Вы заблокированы.\nПричина: {reason}")
                except: pass
            await session.commit()
