...
from sqlalchemy import select   # <-- ВОТ ИМПОРТ, КОТОРЫЙ БЫЛ ПРОПУЩЕН
...
# ========== ♻️ ЗАМЕНА ЛОГА ==========
@router.message(F.text == "♻️ Замена лога")
...
async def replace_photos_done(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for admin_id in ADMIN_IDS:
            try:
                caption = (f"♻️ Заявка на замену лога #{req.id}\n"
...
async def show_categories(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if cats: await message.answer("Выберите категорию:", reply_markup=categories_keyboard(cats))
        else: await message.answer("В каталоге пока нет товаров.")
...
async def category_selected(callback: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if subcats: await callback.message.edit_text("Выберите подкатегорию:", reply_markup=categories_keyboard(subcats))
        else:
            ...
            else: await callback.answer("В этой категории пока нет товаров", show_alert=True)
...
async def buy_start(callback: CallbackQuery, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not product or not product.is_available: await callback.answer("Товар недоступен", show_alert=True); return
        ...
        await state.set_state(BuyProduct.amount); await state.update_data(product_id=product_id)
    await callback.answer()
...
async def buy_amount(message: Message, state: FSMContext):
    ...
    data = await state.get_data(); product_id = data['product_id']; user_id = message.from_user.id
    ...
    async with AsyncSessionLocal() as session:
        ...
        else: await message.answer(f"❌ {result['error']}")
    await state.clear()
...
async def ask_amount(message: Message, state: FSMContext):
    await clear_state_on_menu(message, state)
    await message.answer("Введите сумму пополнения в $:"); await state.set_state(ReplenishBalance.amount)
...
async def process_amount(message: Message, state: FSMContext):
    ...
    await message.answer(f"💳 Счёт на {amount}$ создан.\nОплатите и нажмите «Проверить оплату».", reply_markup=payment_keyboard(inv['pay_url']))
    await state.clear()
...
async def check_payment(callback: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if data and data['status'] == 'paid':
            inv.status = 'paid'; user = await get_user(session, user_id); user.balance += inv.amount
            await session.commit(); await callback.message.answer(f"✅ Зачислено {inv.amount:.2f}$")
            await log_refill(callback.bot, user_id, callback.from_user.username, inv.amount)
        elif data and data['status'] == 'expired': inv.status = 'expired'; await session.commit(); await callback.message.answer("⌛ Счёт истёк.")
...
async def promocode_start(message: Message, state: FSMContext):
    await clear_state_on_menu(message, state)
    await message.answer("Введите промокод:"); await state.set_state(PromocodeInput.code)

@router.message(PromocodeInput.code)
async def promocode_apply(message: Message, state: FSMContext):
    code = message.text.strip(); user_id = message.from_user.id
    async with AsyncSessionLocal() as session:
        user = await get_user(session, user_id)
        ...
        if not promo or not promo.is_active: await message.answer("❌ Недействителен.")
            ...
            user.balance += promo.bonus_amount; promo.used_count += 1; used.append(code); user.used_promocodes = ','.join(used)
            await session.commit(); await message.answer(f"🎁 Промокод +{promo.bonus_amount:.2f}$")
...
async def unban_cancel(callback: CallbackQuery, state: FSMContext): await callback.message.answer("Отменено."); await state.clear()
