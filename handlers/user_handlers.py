...
from keyboards.kb import (
    main_menu, to_main, categories, products, product_view, amount_pick,
    profile, choose_asset, pay_link, history, purchase_detail, review_rating,
    appeal, unban_confirm, replace_action, unban_action, after_spin
)
from utils.states import BuyState, TopupState, PromoState, ReviewState, ReplaceState, UnbanState
...
async def get_discount(session, uid):
    return (await session.execute(
        select(UserDiscount).where(UserDiscount.user_id == uid, UserDiscount.expires_at > datetime.utcnow())
    )).scalar_one_or_none()

async def show_main(target, text: str = None):
    t = f"{pe('crown')} <b>Главное меню</b>"
    if isinstance(target, Message):
        if text:
            await target.answer(text, parse_mode="HTML")
        await target.answer(t, parse_mode="HTML", reply_markup=main_menu())
    else:  # CallbackQuery
        try:
            await target.message.edit_text(t, parse_mode="HTML", reply_markup=main_menu())
        except Exception:
            await target.message.answer(t, parse_mode="HTML", reply_markup=main_menu())

async def safe_edit(callback: CallbackQuery, text: str, kb=None, parse_mode="HTML"):
    try:
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as s:
        user, is_new = await get_or_create_user(s, message.from_user.id, message.from_user.username, ref_code)
        if is_new:
            await log_register(message.bot, message.from_user.id, message.from_user.username, user.referred_by)
            if user.referred_by:
                try:
                    await message.bot.send_message(
                        user.referred_by,
                        f"{pe('gift')} По вашей реф. ссылке зарегистрировался новый пользователь!",
                        parse_mode="HTML"
                    )
                except Exception: pass

    await show_main(message, f"{pe('crown')} Добро пожаловать!")
...
async def cb_catalog(callback: CallbackQuery):
    ...
    if not cats:
        await callback.answer("Каталог пуст", show_alert=True); return
...
async def cb_cats_back(callback: CallbackQuery):
    async with AsyncSessionLocal() as s:
        cats = await get_categories(s)
    await safe_edit(callback, f"{pe('folder')} <b>Каталог</b>", categories(cats) if cats else to_main())
    await callback.answer()

@router.callback_query(F.data.startswith("prod_"))
async def cb_prod(callback: CallbackQuery, state: FSMContext):
...
async def cb_quick_amount(callback: CallbackQuery, state: FSMContext):
    _, pid, n = callback.data.split("_")
    await state.clear()
    await _do_buy(int(pid), int(n), callback.from_user.id, callback.from_user.username, callback.message, callback.bot)
    await callback.answer()
...
async def _do_buy(pid, amount, uid, username, msg_obj, bot):
    ...
    if res["success"]:
        ...
        if res.get("content"):
            await msg_obj.answer(f"{pe('books')} <b>Товар:</b>\n\n<code>{res['content']}</code>", parse_mode="HTML")
        if res.get("file_id"):
        ...
        await msg_obj.answer(f"{pe('warning')} {res['error']}", parse_mode="HTML", reply_markup=to_main())
...
async def cb_notify(callback: CallbackQuery):
    ...
    async with AsyncSessionLocal() as s:
        ex = (await s.execute(select(StockNotify).where(StockNotify.user_id==uid, StockNotify.product_id==pid))).scalar_one_or_none()
...
async def cb_reviews(callback: CallbackQuery):
    ...
    async with AsyncSessionLocal() as s:
        p = await s.get(Product, pid)
        revs = (await s.execute(select(Review).where(Review.product_id==pid).order_by(Review.created_at.desc()).limit(10))).scalars().all()
...
async def show_profile(target):
    ...
    if isinstance(target, CallbackQuery):
        try: await target.message.edit_text(text, parse_mode="HTML", reply_markup=profile())
        except Exception: await target.message.answer(text, parse_mode="HTML", reply_markup=profile())
    else:
        await target.answer(text, parse_mode="HTML", reply_markup=profile())

@router.callback_query(F.data == "m_profile")
async def cb_profile2(callback: CallbackQuery):
    await show_profile(callback); await callback.answer()
...
async def cb_ref(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        f"<code>{link}</code>\n\n"
        f"За каждое пополнение реферала вы получаете <b>{REF_BONUS_PCT}%</b> на баланс!\n"
...
async def cb_topup(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(f"{pe('wallet')} Выбери валюту:", parse_mode="HTML", reply_markup=choose_asset())
...
async def cb_asset(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"{pe('wallet')} Валюта: <b>{asset}</b>\n\nВведи сумму в $:", parse_mode="HTML")
    await callback.answer()
...
async def msg_topup_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"{pe('warning')} Ошибка: {e}", parse_mode="HTML")
...
async def cb_check_pay(callback: CallbackQuery):
    ...
    async with AsyncSessionLocal() as s:
        ...
        if data and data["status"] == "paid":
            ...
            # Реф бонус — 10% от суммы пополнения
            if user.referred_by:
                ...
                if referrer:
                    ...
                    try:
                        await callback.bot.send_message(
                            referrer.user_id,
                            f"{pe('gift')} Реферальный бонус: <b>+{bonus:.4f}$</b>\n"
                            f"({REF_BONUS_PCT}% от пополнения вашего реферала на {inv.amount:.2f}$)",
                            parse_mode="HTML"
                        )
                    except Exception: pass
            await s.commit()
            ...
            await callback.message.answer(f"{pe('clock')} Счёт истёк.", parse_mode="HTML", reply_markup=to_main())
        else:
...
async def msg_promo(message: Message, state: FSMContext):
    ...
    if not code:
        await message.answer("Промокод не может быть пустым."); return
    async with AsyncSessionLocal() as s:
        ...
        now = datetime.utcnow()
        if code in used:
            await message.answer(f"{pe('ban')} Этот промокод уже использован.", parse_mode="HTML")
        elif not promo or not promo.is_active:
            await message.answer(f"{pe('ban')} Промокод не найден или деактивирован.", parse_mode="HTML")
        elif promo.expires_at and promo.expires_at < now:
            await message.answer(f"{pe('clock')} Промокод истёк.", parse_mode="HTML")
        elif promo.max_activations is not None and promo.used_count >= promo.max_activations:
            await message.answer(f"{pe('ban')} Лимит активаций исчерпан.", parse_mode="HTML")
        else:
...
async def cb_rev_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(f"{pe('star')} Оцени товар:", parse_mode="HTML", reply_markup=review_rating(pid))
    await callback.answer()
...
async def msg_review_text(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as s:
        ...
        if prod:
            ...
            prod.rating_count += 1
        await s.commit()
    await message.answer(f"{pe('check')} Отзыв сохранён! {'⭐'*data['rating']}", parse_mode="HTML", reply_markup=to_main())
    await state.clear()
...
async def cb_discount(callback: CallbackQuery):
    ...
    async with AsyncSessionLocal() as s:
        ...
        if old: old.percent = percent; old.expires_at = expires
        else: s.add(UserDiscount(user_id=uid, percent=percent, expires_at=expires))
        await s.commit()
...
async def cb_replace_start(callback: CallbackQuery, state: FSMContext):
    async with AsyncSessionLocal() as s:
        ex = (await s.execute(select(Purchase).where(Purchase.user_id==callback.from_user.id))).first()
    if not ex: await callback.answer("Нет покупок.", show_alert=True); return
...
async def msg_replace_photo(message: Message, state: FSMContext):
    ...
    if len(photos) >= 5: await message.answer("Максимум 5 фото."); return
    photos.append(message.photo[-1].file_id)
    await state.update_data(photos=photos)
    await message.answer(f"Фото {len(photos)}/5.")

@router.message(ReplaceState.photos, F.text)
async def msg_replace_photos_done(message: Message, state: FSMContext):
...
async def msg_replace_text(message: Message, state: FSMContext):
    ...
    await message.answer(f"{pe('check')} Заявка отправлена.", parse_mode="HTML", reply_markup=to_main())
    await state.clear()
...
async def msg_unban_reason(message: Message, state: FSMContext):
    ...
    await message.answer("Отправить заявку?", reply_markup=unban_confirm())
...
async def cb_unban_send(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(f"{pe('check')} Заявка отправлена.", parse_mode="HTML", reply_markup=to_main())
    await state.clear()
    await callback.answer()

# ── ОБРАБОТКА ОТВЕТОВ НА ЗАМЕНУ/РАЗБАН (для пользователей) ───────────────────
@router.callback_query(F.data.startswith("ra_"))  # replace approve
...
@router.callback_query(F.data.startswith("rr_"))  # replace reject
...
@router.callback_query(F.data.startswith("ua_"))  # unban approve
async def cb_ua(callback: CallbackQuery):
    ...
    async with AsyncSessionLocal() as s:
        ...
        if req:
            ...
            await s.commit()
            try:
                await callback.bot.send_message(req.user_id, f"{pe('unlock')} Вы разблокированы!", parse_mode="HTML", reply_markup=to_main())
            except Exception: pass
    await callback.message.answer(f"{pe('check')} Разблокировано.")
    await callback.answer()

@router.callback_query(F.data.startswith("ur_"))  # unban reject
async def cb_ur(callback: CallbackQuery):
    ...
    async with AsyncSessionLocal() as s:
        ...
        if req:
            ...
            try:
                await callback.bot.send_message(req.user_id, f"{pe('ban')} В разблокировке отказано.", parse_mode="HTML", reply_markup=appeal())
            except Exception: pass
    await callback.message.answer(f"{pe('check')} Отклонено.")
    await callback.answer()

from utils.states import ReplaceApprove, ReplaceReject
...
async def msg_ra(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as s:
        ...
        if req: req.status = "approved"; req.admin_comment = message.text; await s.commit()
            try: await message.bot.send_message(req.user_id, f"{pe('check')} Замена #{req.id} одобрена.\n{message.text}", parse_mode="HTML", reply_markup=to_main())
            except Exception: pass
    await message.answer(f"{pe('check')} Одобрено."); await state.clear()
...
async def msg_rr(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as s:
        req = await s.get(ReplaceRequest, data["req_id"])
        if req: req.status = "rejected"; req.admin_comment = message.text; await s.commit()
            try: await message.bot.send_message(req.user_id, f"{pe('ban')} Замена #{req.id} отклонена.\n{message.text}", parse_mode="HTML", reply_markup=to_main())
            except Exception: pass
    await message.answer(f"{pe('check')} Отклонено."); await state.clear()

# ── ЛЮБОЕ НЕИЗВЕСТНОЕ СООБЩЕНИЕ → МЕНЮ ───────────────────────────────────────
@router.message(F.text & ~F.text.startswith("/"))
async def unknown_msg(message: Message, state: FSMContext):
    if await state.get_state(): return  # в FSM состоянии — не мешаем
